...
that the model is now age and sex adjusted. Furthermore, serum creatinine and
urine albumin tend to have skewed distributions in people with kidney disease,
thus we apply logarithm to mitigate the effect of the long tails. Note how we
use the original unadjusted data as input `data = dataset` but the evaluation
is done via the adjusted model:
...
It is possible to use web browsers for a better user experience. At the
time of writing, up-to-date versions of Opera and Google Chrome worked well,
whereas Safari and Firefox did not support all the graphical features.

To view the map colorings, the plots need to be saved in a file first. Here,
...
**Subgroup**: We expect that most uses of Numero will result in the subgrouping of a complex dataset. Visually, we define a subgroup via a contiguous set of adjacent districts on the map. Consequently, all the data points that are located within the set of districts are the subgroup members. Of note, our concept of a subgroup is different from _subgroup discovery_ in data mining; we use the term as a replacement to the word 'cluster' to emphasize the lack of intrinsic structure in the original data space.
