 ...
 */
string
Network::insert(const string& a, const string& b, const mdreal w) {
 ...
 */
mdsize
Network::insert(const Network& net) {
  ...
  for(mdsize i = 0; i < elem.size(); i++) {
    ...
    string err = this->insert(a, b, elem[i].value);
    nedges += (err.size() == 0);
  }
